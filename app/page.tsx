 ...
 return <main>
  <section className="bg-[#edf8ef] border-b border-green-100"><div className="container py-14 md:py-20"><div className="max-w-3xl">
   ...
   <div className="card shadow mt-8 p-2 flex flex-col md:flex-row gap-2">
    <div className="flex-1 relative min-w-0"><Search aria-hidden="true" className="search-icon absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none"/><input aria-label="Cari di Lumagada" className="input search-input" value={q} onChange={e=>setQ(e.target.value)} placeholder="Cari apa aja..."/></div>
    <div className="md:w-48 relative"><MapPin aria-hidden="true" className="search-icon absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none"/><select aria-label="Pilih lokasi" className="input search-location-input appearance-none" value={loc} onChange={e=>setLoc(e.target.value)}><option>Indonesia</option><option>Jakarta</option><option>Bandung</option><option>Surabaya</option><option>Yogyakarta</option></select></div>
  ...
  <section className="container pb-12"><div className="card p-6 md:p-8 bg-white"><div className="grid md:grid-cols-3 gap-6"><div><b className="text-lg">Iklan gratis 7 hari</b><p className="text-gray-500 text-sm mt-1">Pasang iklan tanpa ribet. Setelah 7 hari, login dan re-up dari akun untuk tayang lagi.</p></div><div><b className="text-lg">Pro Seller</b><p className="text-gray-500 text-sm mt-1">Seller terdaftar dengan kredensial lengkap untuk membangun kepercayaan dan transaksi lebih serius.</p></div><div><b className="text-lg">Transaksi lebih nyaman</b><p className="text-gray-500 text-sm mt-1">Gunakan chat Lumagada, cek profil seller, dan pilih metode transaksi yang sesuai.</p></div></div></div></section>
