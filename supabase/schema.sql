...
create table if not exists profiles(
 ...
 verified boolean not null default false, seller_type text not null default 'regular' check(seller_type in('regular','pro')),
...
create table if not exists listings(
 ...
 urgent boolean not null default false, is_pro_listing boolean not null default false, accepts_offers boolean not null default true,
...
create unique index if not exists sponsor_reward_daily_idx on sponsor_reward_events(user_id,listing_id,date_trunc('day',created_at));
...
drop policy if exists "conversations create" on conversations; create policy "conversations create" on conversations for insert with check(buyer_id=auth.uid() and exists(select 1 from listings l where l.id=listing_id and l.seller_id=seller_id));
...
drop policy if exists "offers participants update" on offers; create policy "offers participants update" on offers for update using(buyer_id=auth.uid() or seller_id=auth.uid()) with check(buyer_id=auth.uid() or seller_id=auth.uid());
...
-- Platform fee is intentionally not exposed in public UI. Configure the actual fee internally before recording real transactions.
